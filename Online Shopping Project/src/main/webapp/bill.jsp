...
try{
	...
	ResultSet rs = st.executeQuery("select sum(total) from cart where email ='"+mail+"' and status = '+bill+'");
	
	while(rs.next()){
		total = rs.getInt(1);
	}
	ResultSet rs2 = st.executeQuery("select * from user inner join cart  where cart.email ='"+mail+"' and cart.status='bill'");
	while(rs2.next()){
%>
